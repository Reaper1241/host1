...
function updateCars() {
    ...
    fetch(`${apiNew}filters/cars?sorting=${appStore.newSort}&per_page=${appStore.currentPerPage}&page=${page.value}${quary.value ? `&${quary.value}` : ''}`)
        .then(res => res.json())
        .then(data => query.value = data)
...
function loadPage() {
    ...
    fetch(`${apiNew}filters/cars?sorting=${appStore.newSort}&per_page=${appStore.currentPerPage}${brand.value ? `&brand_id=${brand.value.id}` : ''}${model.value ? `&car_model_id=${model.value.id}` : ''}&page=${page.value}${quary.value ? `&${quary.value}` : ''}`)
        .then(res => res.json())
        .then(data => query.value = data)
...
const title = computed(() => {
    if ($route.params.car) {
        ...
        return `Авто ${cleanUpTitle($route.params.brand, $route.params.model)} в наличии`
    } else if ($route.params.brand) {
        return `Авто ${cleanUpTitle($route.params.brand)} в наличии`
    } else {
        return 'Авто в наличии'
    }
})
...
<template>
    <section :ref="(el) => { appStore.newCatalog = el }" class="cars section" id="model__cars" v-if="checkCars && carsCount">
        <div class="cars__content" v-if="!carsLoading">
            <div class="container head__title">
                <CarsNewHead :cars-count="carsCount - ($route.params.car ? 1 : 0)" @update-cars="updateCars()"
                    :title="`${title}`" />
            </div>
            <div class="cars__container">
                <div class="container">
                    <div class="cars__body cars-template">
                        <CarSmallC1 v-for="car in cars" :car="car" :key="car" />
                    </div>
                    <div class="cars__more" v-if="totalPages > page">
                        <button class="cars__more-button standart-button" @click="loadPage()">Загрузить ещё</button>
                    </div>
                </div>
            </div>
        </div>
